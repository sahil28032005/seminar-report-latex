...
\usepackage{graphicx} % Correctly included

\title{Sample Document with Modular Errors}
\author{Sahil Suhas Sadekar}
\date{October 2024}

\begin{document}
\maketitle

\input{introduction.tex} % This file will cause an error
\input{section_with_error.tex} % This will also cause an error
\input{missing_file.tex} % This file does not exist
...
This is a sample document to demonstrate LaTeX errors. 
...
% Missing closing tag for a list
\begin{itemize}
    \item First item
    \item Second item
    % Missing \end{itemize}
    
% Extra text after end command
\end{document} Some extra text.
